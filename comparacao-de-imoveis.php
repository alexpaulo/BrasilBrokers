<!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
        <title>Brasil Brokers</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=1000px">
        
        <link href="fav.ico" rel="icon" type="image/x-icon" />
        <link rel="stylesheet" href="css/normalize.min.css">
        <link rel="stylesheet" href="css/main.css">

        <!--[if lt IE 9]>
            <script src="//html5shiv.googlecode.com/svn/trunk/html5.js"></script>
            <script>window.html5 || document.write('<script src="js/vendor/html5shiv.js"><\/script>')</script>
        <![endif]-->
    </head>
    <body class="comparacao">
        <!--[if lt IE 7]>
            <p class="chromeframe">You are using an <strong>outdated</strong> browser. Please <a href="http://browsehappy.com/">upgrade your browser</a> or <a href="http://www.google.com/chromeframe/?redirect=true">activate Google Chrome Frame</a> to improve your experience.</p>
        <![endif]-->
        <div id="master">
            <?php include("include/header.php") ?>

            <section class="section">
                <div class="wrap">
                    <div class="row">
                        <div class="breadcrumb">
                            <ol class="crumbs">
                                <li class="first">
                                    <a href="#">
                                        <span>Home</span>
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <span>Resultado de Busca</span>
                                    </a>
                                </li> 
                                <li>
                                    <a href="#">
                                        <span>Comparação de Imóveis</span>
                                    </a>
                                </li>  
                            </ol><!-- /breadcrumb -->
                        </div><!-- /breadcrumb -->

                        <div class="topShare">
                            <span class="fb">
                                <div id="fb-root"></div>
                                <script>(function(d, s, id) {
                                  var js, fjs = d.getElementsByTagName(s)[0];
                                  if (d.getElementById(id)) return;
                                  js = d.createElement(s); js.id = id;
                                  js.src = "//connect.facebook.net/pt_BR/all.js#xfbml=1";
                                  fjs.parentNode.insertBefore(js, fjs);
                                }(document, 'script', 'facebook-jssdk'));</script>
                                <div class="fb-like" data-href="https://www.brasilbrokers.com.br" data-send="false" data-layout="button_count" data-width="150" data-show-faces="false"></div>
                            </span>
                            <span class="gplus">
                                <!-- Place this tag where you want the +1 button to render. -->
                                <div class="g-plusone" data-size="medium" data-href="http://www.brasilbrokers.com.br"></div>

                                <!-- Place this tag after the last +1 button tag. -->
                                <script type="text/javascript">
                                  window.___gcfg = {lang: 'pt-BR'};

                                  (function() {
                                    var po = document.createElement('script'); po.type = 'text/javascript'; po.async = true;
                                    po.src = 'https://apis.google.com/js/plusone.js';
                                    var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(po, s);
                                  })();
                                </script>
                            </span>
                        </div><!-- /topShare -->
                    </div><!-- /row -->
                    
                    <article class="article">

                        <h1 class="titleDefault">
                            Comparação <span class="verde">de Imóveis</span>
                        </h1>
                        
                        <div class="box_default row">   
                            
                            <div class="row bar_links">
                                <a href="#" class="icoPrinter002"></a>
                                <a href="#" class="bt_voltar"> « VOLTAR</a>
                            </div><!-- /row -->

                            <ul class="lista_comparacao row">
                                <li>
                                    <table>
                                        <tr>
                                            <th>
                                                <img class="img_capa" src="img/comparacao_capa.jpg" alt="">
                                            </th>
                                        </tr>

                                        <tr>
                                            <td>
                                                Bairro
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Cidade
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Dormitórios
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Suites
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Vagas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Valor
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Playground
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Salão de Festas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Piscina
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Salão de Jogos
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Fitness
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Plauground
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                Quadra
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <h4>Legenda:</h4>
                                                <div class="ico_pos txt">Possui</div>
                                                <div class="ico_neg txt">Não Possui</div>
                                            </td>
                                        </tr>
                                    </table>
                                </li>
                                <li>
                                    <table>
                                        <tr>
                                            <th>
                                                <a href="#" class="ico_excluir">excluir</a>
                                                <h2 class="title">
                                                    recém reformado, lazer completo
                                                </h2>
                                                
                                                <div class="wrapImg row">
                                                    <div class="thatsImg">
                                                        <img width="191" height="129" alt="" src="img/exemploIMGimovel001.jpg">
                                                    </div>

                                                    <div class="thatsLabel exclusivo"></div>
                                                    <div class="thatsLabel lancamento"></div>
                                                </div>
                                            </th>
                                        </tr>
                                        <tr>
                                            <td>
                                                Santana
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                São Paulo
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                1,2 ou 3 Dorms
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                sendo 2 suítes
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                3 Vagas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="valor">
                                                R$ 986.000,00                                                
                                            </td>
                                        </tr>


                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>

                                        <tr>
                                            <td>
                                                <a class="btAzul002" title="" href="#">
                                                    <span>Mais detalhes</span>
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
                                </li>

                                <li>
                                    <table>
                                        <tr>
                                            <th>
                                                <a href="#" class="ico_excluir">excluir</a>
                                                <h2 class="title">
                                                    RESIDENCIAL BARCELONETA
                                                </h2>
                                                
                                                <div class="wrapImg row">
                                                    <div class="thatsImg">
                                                        <img width="191" height="129" alt="" src="img/exemploIMGimovel001.jpg">
                                                    </div>

                                                    <div class="thatsLabel pronto"></div>
                                                </div>
                                            </th>
                                        </tr>
                                        <tr>
                                            <td>
                                                Santana
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                São Paulo
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                1,2 ou 3 Dorms
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                sendo 2 suítes
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                3 Vagas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="valor">
                                                valor sob consulta                                               
                                            </td>
                                        </tr>


                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>

                                        <tr>
                                            <td>
                                                <a class="btAzul002" title="" href="#">
                                                    <span>Mais detalhes</span>
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
                                </li>

                                <li>
                                    <table>
                                        <tr>
                                            <th>
                                                <a href="#" class="ico_excluir">excluir</a>
                                                <h2 class="title">
                                                    RESIDENCIAL BARCELONETA
                                                </h2>
                                                
                                                <div class="wrapImg row">
                                                    <div class="thatsImg">
                                                        <img width="191" height="129" alt="" src="img/exemploIMGimovel001.jpg">
                                                    </div>

                                                    <div class="thatsLabel exclusivo"></div>
                                                    <div class="thatsLabel breve"></div>
                                                </div>
                                            </th>
                                        </tr>
                                        <tr>
                                            <td>
                                                Santana
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                São Paulo
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                1,2 ou 3 Dorms
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                sendo 2 suítes
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                3 Vagas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="valor">
                                                R$ 986.000,00                                                
                                            </td>
                                        </tr>


                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>

                                        <tr>
                                            <td>
                                                <a class="btAzul002" title="" href="#">
                                                    <span>Mais detalhes</span>
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
                                </li>

                                <li>
                                    <table>
                                        <tr>
                                            <th>
                                                <a href="#" class="ico_excluir">excluir</a>
                                                <h2 class="title">
                                                    RESIDENCIAL BARCELONETA
                                                </h2>
                                                
                                                <div class="wrapImg row">
                                                    <div class="thatsImg">
                                                        <img width="191" height="129" alt="" src="img/exemploIMGimovel001.jpg">
                                                    </div>

                                                    <div class="thatsLabel exclusivo"></div>
                                                    <div class="thatsLabel lancamento"></div>
                                                </div>
                                            </th>
                                        </tr>
                                        <tr>
                                            <td>
                                                Santana
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                São Paulo
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                1,2 ou 3 Dorms
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                sendo 2 suítes
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                3 Vagas
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="valor">
                                                R$ 986.000,00                                                
                                            </td>
                                        </tr>


                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_pos"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="ico_neg"></div>
                                            </td>
                                        </tr>

                                        <tr>
                                            <td>
                                                <a class="btAzul002" title="" href="#">
                                                    <span>Mais detalhes</span>
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
                                </li>
                            </ul>
                            
                        </div><!-- /box_default -->
                    </article>
                    
                </div><!-- /wrap -->
            </section><!-- /section -->

            <?php include("include/footer.php") ?>
        </div><!-- /master -->
        <?php include("include/scripts.php") ?>
    </body>
</html>
